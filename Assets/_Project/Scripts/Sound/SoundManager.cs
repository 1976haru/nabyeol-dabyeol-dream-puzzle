...
// Task 101 — Extended with BGM loop playback, fade transitions, separate
//            bgmVolume, and BgmType-based dispatch.
//
// Design:
//   - Singleton. Separate AudioSources for SFX (PlayOneShot) and BGM (loop).
//   - SFX and BGM are bound by enum so callers don't pass strings. Clips
//     are configured in the Inspector. Missing clips degrade gracefully
//     (warning once per type, no throw).
//   - Volume and mute are persisted to PlayerPrefs and reloaded on Awake.
//   - Final SFX volume = master × sfx × clip.volume.
//     Final BGM volume = master × bgm × clip.volume.
//   - PlayBgm/StopBgm switch instantly. PlayBgmWithFade does a coroutine-
//     based fade-out/fade-in for smooth scene transitions.
//   - Same-type PlayBgm calls while already playing are no-ops, so repeated
//     UI inits don't restart the music.
...
namespace NabyeolDabyeolDreamPuzzle.Sound
{
    ...
    /// <summary>
    /// Central manager for all gameplay SFX + BGM. Persisted as a singleton
    /// across scenes (DontDestroyOnLoad) so audio survives scene reloads.
    /// </summary>
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        /// when the clip is missing, or when the source is gone.
        /// </summary>
