...
//
// Defensive: null target → yield break. Disabled component → yield break.
// Sound is *not* triggered here — SoundManager hooks already exist at the
// same gameplay sites; duplicating would double the sound.
...
namespace NabyeolDabyeolDreamPuzzle.Animation
{
    ...
    public class SimpleAnimationManager : MonoBehaviour
    {
        ...
        [Header("Block pop")]
        [Tooltip("Total duration of the scale-up + scale-down pop, in seconds.")]
        ...
        /// <summary>
        /// Scale-up to peak then collapse to a tiny scale. Caller is expected to
        /// destroy the block after the routine finishes. No scale restore — the
        /// shrunk state should remain so the visual doesn't snap back before destroy.
        /// </summary>
        public IEnumerator PlayBlockPop(Transform target)
        {
            ...
            // down (to a tiny scale, ready for destroy)
            t = 0f;
            while (t < half)
            {
                if (target == null) yield break;
        ...
        /// <summary>One-shot scale 1 → peak → 1 pulse. Restores original scale at end.</summary>
        ...
        /// <summary>Two-stage bounce: up to peak quickly, settle back. Restores original scale.</summary>
        public IEnumerator PlayCharacterBounce(RectTransform target)
        {
            ...
            // Quick up (1/3 of the duration), softer settle (2/3).
        ...
        /// <summary>Position shake within strength radius. Restores original anchored position.</summary>
        ...
        /// <summary>Lerp the Image's alpha from <paramref name="from"/> to <paramref name="to"/>.</summary>
        ...
        /// <summary>
        /// Convenience: fade out → swap sprite → fade in. Sprite swap happens
        /// at the midpoint so the change is invisible during the dim moment.
        /// </summary>
        ...
        /// <summary>Scale in from startScale to 1. Restores the target's prior scale at end.</summary>
