...
export function MenuScreen({ onNavigate }: Props) {
  ...
  return (
    <div style={wrap}>
      ...
      <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: 18, marginTop: 8 }}>{theme.menuSubtitle}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', maxWidth: 300, marginTop: 12 }}>
        ...
        {theme.hasQuiz && (
          <button type="button" style={secondary} onClick={() => onNavigate('quiz')}>
            ⚖️ 법 퀴즈
          </button>
        )}
        {theme.hasParentReport && (
          <button type="button" style={reportBtn} onClick={() => onNavigate('report')}>
            👪 부모 학습 보고서
          </button>
        )}
      </div>
